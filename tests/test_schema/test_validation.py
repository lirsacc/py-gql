...
from py_gql.schema import (
    Argument, Schema, ScalarType, ObjectType, InterfaceType, UnionType,
    EnumType, InputObjectType, ListType, NonNullType, String, Field, EnumValue,
    InputField, validate_schema, Int)
...
SomeObject = ObjectType('SomeObject', [
    Field('f', String)
])
...
SomeInterface = InterfaceType('SomeInterface', [
    Field('f', String)
])

SomeEnum = EnumType('SomeEnum', [EnumValue('ONLY')])

SomeInputObject = InputObjectType('SomeInputObject', [
...
def _single_type_schema(typ, fieldname='f'):
    return Schema(
        ObjectType('Query', [Field(fieldname, typ)]),
        types=[typ]
    )
...
def test_reject_object_with_incorrectly_named_fields():
    schema = Schema(
        ObjectType('Query', [
            Field('bad-name-with-dashes', String)
        ])
    )
...
def test_reject_incorrectly_named_type():
    ...
    assert (str(exc_info.value) ==
            'Invalid type name "bad-name-with-dashes", '
            'must match /^[_a-zA-Z][_a-zA-Z0-9]*$/')
...
@pytest.mark.parametrize('typ', not_output_types + [None],
                         ids=lambda t: "type = %s" % t)
...
def test_reject_object_implementing_same_interface_twice():
    ...
    assert (str(exc_info.value) ==
            'Type "SomeObject" mut only implement interface '
            '"SomeInterface" once')
...
def test_accept_argument_with_input_type(typ):
    schema = _single_type_schema(
        ObjectType('GoodObject', [
            Field('f', String, [Argument('goodArg', typ)])
        ])
    )
    assert schema.validate()


@pytest.mark.parametrize('typ', not_input_types + [None],
                         ids=lambda t: "type = %s" % t)
def test_reject_argument_with_non_input_type(typ):
    schema = _single_type_schema(
        ObjectType('BadObject', [
            Field('f', String, [Argument('badArg', typ)])
        ])
    )
    with pytest.raises(SchemaError) as exc_info:
        validate_schema(schema)
...
def test_accept_input_object_with_input_type(typ):
    schema = _single_type_schema(
        ObjectType('GoodObject', [
            Field('f', String, [
                Argument('goodArg', InputObjectType('GoodInput', [
                    InputField('f', typ)
                ])
                )
            ])
...
def test_reject_input_object_with_non_input_type(typ):
    schema = _single_type_schema(
        ObjectType('BadObject', [
            Field('f', String, [
                Argument('badArg', InputObjectType('BadInput', [
                    InputField('f', typ)
                ])
                )
            ])
...
def test_reject_input_object_with_no_field():
    ...
    assert (str(exc_info.value) ==
            'Type "BadInput" must define at least one field')


def test_accept_object_which_implements_interface():
    schema = _single_type_schema(ObjectType('SomeObject', [
        Field('f', String)
...
def test_reject_object_fields_with_missing_interface_argument():
    ...
    assert (str(exc_info.value) ==
            'Interface field argument "IFace.f.arg" is not '
            'provided by "Obj.f"')
