...
def _single_type_schema(typ, fieldname="f"):
    return Schema(ObjectType("Query", [Field(fieldname, typ)]), types=[typ])
...
def test_reject_non_object_mutation_type():
    schema = Schema(ObjectType("Query", [Field("test", String)]), mutation_type=String)
...
def test_reject_non_object_subscription_type():
    ...
    assert str(exc_info.value) == 'Subscription must be ObjectType but got "String"'
...
def test_reject_object_without_fields():
    ...
    assert (
        str(exc_info.value) == 'Type "IncompleteObject" must define at least one field'
    )


def test_reject_object_with_incorrectly_named_fields():
    schema = Schema(ObjectType("Query", [Field("bad-name-with-dashes", String)]))
...
def test_accept_field_args_with_correct_names():
    schema = _single_type_schema(
        ObjectType(
            "SomeObject", [Field("field", String, [Argument("goodArg", String)])]
        )
    )
    assert validate_schema(schema)
...
def test_reject_field_args_with_incorrect_names():
    schema = _single_type_schema(
        ObjectType(
            "SomeObject", [Field("field", String, [Argument("bad-arg", String)])]
        )
    )
...
def test_reject_union_type_with_no_member():
    ...
    assert (
        str(exc_info.value) == 'UnionType "EmptyUnion" must at least define one member'
    )
...
def test_reject_union_type_with_duplicate_members():
    ...
    assert (
        str(exc_info.value) == 'UnionType "BadUnion" can only include type "TypeA" once'
    )


def test_reject_union_type_with_non_object_members():
    TypeA = ObjectType("TypeA", [Field("f", String)])
    BadUnion = UnionType("BadUnion", [TypeA, SomeScalar])
    schema = _single_type_schema(BadUnion)
    with pytest.raises(SchemaError) as exc_info:
        validate_schema(schema)
...
def test_accept_input_type():
    schema = _single_type_schema(
        ObjectType(
            "Object", [Field("field", String, [Argument("arg", SomeInputObject)])]
        )
    )
    assert schema.validate()
...
def test_reject_input_type_with_no_fields():
    ...
    schema = _single_type_schema(
        ObjectType("Object", [Field("field", String, [Argument("arg", EmptyInput)])])
    )

    with pytest.raises(SchemaError) as exc_info:
        validate_schema(schema)

    assert str(exc_info.value) == 'Type "EmptyInput" must define at least one field'


def test_reject_input_type_with_incorectly_typed_fields():
    BadInput = InputObjectType("BadInput", [InputField("f", SomeObject)])
    schema = _single_type_schema(
        ObjectType("Object", [Field("field", String, [Argument("arg", BadInput)])])
    )
    ...
    assert (
        str(exc_info.value) == 'Expected input type for field "f" on "BadInput" '
        'but got "SomeObject"'
    )
...
def test_reject_enum_type_with_no_values():
    ...
    assert str(exc_info.value) == 'EnumType "EmptyEnum" must at least define one value'
...
@pytest.mark.parametrize("typ", output_types, ids=lambda t: "type = %s" % t)
def test_accept_output_type_as_object_fields(typ):
    schema = _single_type_schema(typ)
    assert schema.validate()


@pytest.mark.parametrize(
    "typ", not_output_types + [None], ids=lambda t: "type = %s" % t
)
def test_reject_non_output_type_as_object_fields(typ):
    schema = _single_type_schema(typ)
    with pytest.raises(SchemaError) as exc_info:
        validate_schema(schema)
    ...
    assert (
        str(exc_info.value)
        == 'Expected output type for field "f" on "Query" but got "%s"' % typ
    )
...
@pytest.mark.parametrize("typ", output_types, ids=lambda t: "type = %s" % t)
def test_accept_interface_fields_with_output_type(typ):
    iface = InterfaceType("GoodInterface", [Field("f", typ)])
...
def test_reject_interface_fields_with_non_output_type(typ):
    iface = InterfaceType("BadInterface", [Field("f", typ)])
    schema = _single_type_schema(iface)
    with pytest.raises(SchemaError) as exc_info:
        validate_schema(schema)

    assert (
        str(exc_info.value) == 'Expected output type for field "f" on "BadInterface" '
        'but got "%s"' % typ
    )
...
def test_reject_interface_with_no_field():
    ...
    assert str(exc_info.value) == 'Type "BadInterface" must define at least one field'


@pytest.mark.parametrize("typ", input_types, ids=lambda t: "type = %s" % t)
def test_accept_argument_with_input_type(typ):
    schema = _single_type_schema(
        ObjectType("GoodObject", [Field("f", String, [Argument("goodArg", typ)])])
    )
    assert schema.validate()


@pytest.mark.parametrize("typ", not_input_types + [None], ids=lambda t: "type = %s" % t)
def test_reject_argument_with_non_input_type(typ):
    schema = _single_type_schema(
        ObjectType("BadObject", [Field("f", String, [Argument("badArg", typ)])])
    )
    with pytest.raises(SchemaError) as exc_info:
        validate_schema(schema)
...
@pytest.mark.parametrize("typ", input_types, ids=lambda t: "type = %s" % t)
def test_accept_input_object_with_input_type(typ):
    schema = _single_type_schema(
        ObjectType(
            "GoodObject",
            [
                Field(
                    ...
                    [
                        Argument(
                            "goodArg",
                            InputObjectType("GoodInput", [InputField("f", typ)]),
                        )
                    ],
                )
            ],
...
@pytest.mark.parametrize("typ", not_input_types + [None], ids=lambda t: "type = %s" % t)
def test_reject_input_object_with_non_input_type(typ):
    schema = _single_type_schema(
        ObjectType(
            "BadObject",
            [
                Field(
                    ...
                    [
                        Argument(
                            "badArg",
                            InputObjectType("BadInput", [InputField("f", typ)]),
                        )
                    ],
                )
            ],
...
def test_reject_input_object_with_no_field():
    schema = _single_type_schema(
        ObjectType(
            "BadObject",
            [Field("f", String, [Argument("badArg", InputObjectType("BadInput", []))])],
        )
    )
    with pytest.raises(SchemaError) as exc_info:
        validate_schema(schema)

    assert str(exc_info.value) == 'Type "BadInput" must define at least one field'


def test_accept_object_which_implements_interface():
    schema = _single_type_schema(
        ObjectType("SomeObject", [Field("f", String)], interfaces=[SomeInterface])
    )
    assert schema.validate()
...
def test_reject_object_missing_interface_field():
    schema = _single_type_schema(
        ObjectType("SomeObject", [Field("_f", String)], interfaces=[SomeInterface])
    )
    with pytest.raises(SchemaError) as exc_info:
        validate_schema(schema)

    assert (
        str(exc_info.value) == 'Interface field "SomeInterface.f" is not implemented '
        'by type "SomeObject"'
    )
...
def test_accept_object_fields_with_interface_subtype_of_interface_field():
    iface = InterfaceType("IFace", [Field("f", lambda: iface)])
    obj = ObjectType("Obj", [Field("f", lambda: obj)], interfaces=[iface])
...
def test_reject_object_fields_with_missing_interface_argument():
    iface = InterfaceType("IFace", [Field("f", String, [Argument("arg", String)])])
...
def test_accept_object_with_list_interface_list_field():
    ...
    schema = _single_type_schema(
        ObjectType("SomeObject", [Field("f", ListType(String))], interfaces=[iface])
    )
    assert schema.validate()
...
def test_accept_object_with_non_list_interface_list_field():
    ...
    assert (
        str(exc_info.value) == 'Interface field "IFace.f" expects type "[String]" but '
...
def test_accept_object_with_non_null_interface_null_field():
    iface = InterfaceType("IFace", [Field("f", String)])
    schema = _single_type_schema(
        ObjectType("SomeObject", [Field("f", NonNullType(String))], interfaces=[iface])
    )
    assert schema.validate()
