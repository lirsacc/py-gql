...
from py_gql.exc import UnknownType, SchemaError
from py_gql.schema import (
    InterfaceType, Field, Schema, String, ObjectType, InputObjectType,
    Directive, Arg, ListType, InputField, Boolean, Int)

Interface = InterfaceType('Interface', [
    Field('fieldName', String),
])

Implementing = ObjectType('Object', [
    Field('fieldName', String),
], interfaces=[Interface])

DirInput = InputObjectType('DirInput', [
    Field('field', String),
])
...
Dir = Directive('dir', ['OBJECT'], [
    Arg('arg', DirInput),
    Arg('argList', ListType(WrappedDirInput)),
])
...
BlogMutation = ObjectType('Mutation', [
    Field('writeArticle', BlogArticle)
])

BlogSubscription = ObjectType('Subscription', [
    Field('articleSubscribe', BlogArticle, [Arg('id', String)])
])
...
def test_Schema_is_possible_type_is_accurate():
    schema = Schema(
        ObjectType('Query', [
            Field('getObject', Interface, resolve=_null_resolver),
        ]),
        directives=[Dir],
    )
...
def test_Schema_includes_nested_input_objects_in_the_map():
    ...
    SomeMutation = ObjectType('SomeMutation', [
        Field('mutateSomething', BlogArticle, [
            Arg('input', SomeInputObject)
        ])
    ])
    SomeSubscription = ObjectType('SomeSubscription', [
        Field('subscribeToSomething', BlogArticle, [
            Arg('input', SomeInputObject)
        ])
    ])
...
def test_Schema_includes_interface_possible_types_in_the_type_map():
    ...
    SomeSubtype = ObjectType('SomeSubtype', [
        Field('f', Int)
