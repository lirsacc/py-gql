...
from py_gql.schema import (
    InterfaceType, ObjectType, Field, Boolean, String, ListType, Schema,
    UnionType)
...
def test_ObjectType_is_type_of_for_interface_runtime_inference():
    PetType = InterfaceType('Pet', [
        Field('name', String),
    ])

    DogType = ObjectType('Dog', [
        Field('name', String),
        Field('woofs', Boolean),
    ], interfaces=[PetType], is_type_of=Dog)

    CatType = ObjectType('Cat', [
        Field('name', String),
        Field('meows', Boolean),
    ], interfaces=[PetType], is_type_of=Cat)
...
DogType = ObjectType('Dog', [
    Field('name', String),
    Field('woofs', Boolean),
], interfaces=[NamedType], is_type_of=Dog)


CatType = ObjectType('Cat', [
    Field('name', String),
    Field('meows', Boolean),
], interfaces=[NamedType], is_type_of=Cat)
...
PersonType = ObjectType('Person', [
    Field('name', String),
    Field('pets', ListType(PetType)),
    Field('friends', lambda: ListType(NamedType)),
], interfaces=[NamedType], is_type_of=Person)
...
def test_it_executes_union_types():
    ...
    }, [], initial_value=_JOHN, _skip_validation=True)
