...
def test_allows_querying_the_schema_for_types(starwars_schema):
    check_execution(starwars_schema, '''
...
def test_intropsection_query():
    empty_schema = Schema(
        ObjectType('QueryRoot', [
            Field('onlyField', String)
        ])
    )

    check_execution(empty_schema, introspection_query(False), expected_data={
        '__schema': {
            'mutationType': None,
    ...
    }, expected_errors=[], _skip_validation=True)


def test_intropsection_on_input_object():
    test_input = InputObjectType('TestInputObject', [
...
def test_it_exposes_descriptions_on_types_and_fields():
    schema = Schema(ObjectType('QueryRoot', [
        Field('onlyField', String)
    ]))
    check_execution(schema, '''
    {
