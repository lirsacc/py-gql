...
from py_gql.exc import UnknownVariable, InvalidValue
from py_gql.lang.parser import parse_value
from py_gql.utilities.value_from_ast import (
    untyped_value_from_ast, typed_value_from_ast)
from py_gql.schema.scalars import Boolean, Int, Float, String, ID, UUID
from py_gql.schema.types import (
    EnumType, EnumValue, NonNullType, ListType, InputObjectType, InputField)
...
class TestUntyped(object):
    ...
    @pytest.mark.parametrize('value,variables,expected', [
    ...
    def test_it_parses_variables(self, value, variables, expected):
        assert (untyped_value_from_ast(parse_value(value), variables) ==
                expected)
...
TestInput = InputObjectType('TestInput', [
...
class TestTyped(object):
    ...
    @pytest.mark.parametrize('type_,value,expected', [
        ...
        (ID, '123456', '123456'),
        (ID, '"123456"', '123456'),
        (UUID, '"aacefca9-41c6-4a01-b372-632b4dc2506c"',
    ...
    def test_it_raises_on_non_nullable_null(self):
        with pytest.raises(InvalidValue):
            typed_value_from_ast(
                parse_value('null'), NonNullType(Boolean), None)

    @pytest.mark.parametrize('type_,value,expected,error', [
    ...
    def test_it_coerces_lists_of_values(
    ...
    @pytest.mark.parametrize('type_,value,expected,error', [
        ...
        (TestInput, '{ int: 123, requiredBool: false }', {
            'int': 123,
            'requiredBool': False,
        }, None),
        (TestInput, '{ bool: true, requiredBool: false }', {
            'int': 42,
            'bool': True,
            'requiredBool': False,
        }, None),
    ...
    @pytest.mark.parametrize('type_,value,variables,expected,error', [
    ...
    @pytest.mark.parametrize('type_,value,variables,expected,error', [
        ...
        # }, None),
        (TestInput, '{ int: $foo, bool: $foo, requiredBool: true }', {},
         None, UnknownVariable),
        (TestInput, '{ requiredBool: $foo }', {}, None, UnknownVariable),
        (TestInput, '{ requiredBool: $foo, bool: $foo }', {'foo': True}, {
            'int': 42,
            'requiredBool': True,
            'bool': True,
        }, None)])
