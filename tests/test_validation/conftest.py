...
from py_gql.schema import (Schema, ObjectType, InterfaceType, UnionType,
                           EnumType, InputObjectType, ListType, NonNullType,
                           Int, Float, String, Boolean, ID, Field, Arg,
                           EnumValue, ScalarType, Directive, InputField)


@pytest.fixture
def schema():

    Being = InterfaceType('Being', [
        Field('name', String, [Arg('surname', Boolean)]),
    ])

    Pet = InterfaceType('Pet', [
        Field('name', String, [Arg('surname', Boolean)]),
    ])

    Canine = InterfaceType('Canine', [
        Field('name', String, [Arg('surname', Boolean)]),
    ])

    DogCommand = EnumType('DogCommand', [
        EnumValue('SIT', 0),
        EnumValue('HEEL', 1),
        EnumValue('DOWN', 2),
    ])

    FurColor = EnumType('FurColor', [
        EnumValue('BROWN', 0),
        EnumValue('BLACK', 1),
        EnumValue('TAN', 2),
        EnumValue('SPOTTED', 3),
    ...
    Dog = ObjectType('Dog', [
        ...
        Field('isHousetrained', Boolean, [
            Arg('atOtherHomes', Boolean, True)
        ]),
    ...
    Intelligent = InterfaceType('Intelligent', [
        Field('iq', Int)
    ])

    Human = ObjectType('Human', lambda: [
        Field('name', String, args=[Arg('surname', Boolean)]),
        Field('iq', Int),
    ...
    ComplexInput = InputObjectType('ComplexInput', [
    ...
    ComplicatedArgs = ObjectType('ComplicatedArgs', [
        Field('intArgField', String, [Arg('intArg', Int)]),
        Field('nonNullIntArgField', String, [
        ...
        Field('stringListArgField', String, [
            Arg('stringListArg', ListType(String)),
        ]),
        Field('stringListNonNullArgField', String, [
            Arg('stringListNonNullArg', ListType(NonNullType(String))),
        ]),
        Field('complexArgField', String, [
            Arg('complexArg', ComplexInput),
        ]),
        Field('multipleReqs', String, [
            Arg('req1', NonNullType(Int)),
            Arg('req2', NonNullType(Int)),
        ]),
        Field('multipleOpts', String, [
            Arg('opt1', Int, 0),
            Arg('opt2', Int, 0),
        ]),
        Field('multipleOptAndReq', String, [
            Arg('req1', NonNullType(Int)),
            Arg('req2', NonNullType(Int)),
            Arg('opt1', Int, 0),
            Arg('opt2', Int, 0),
...
@pytest.fixture
def schema_2():
    SomeBox = InterfaceType('SomeBox', [
        Field('deepBox', lambda: SomeBox),
        Field('unrelatedField', String),
    ])

    StringBox = ObjectType('StringBox', [
        Field('scalar', String),
    ...
    NonNullStringBox1Impl = ObjectType('NonNullStringBox1Impl', [
        Field('scalar', NonNullType(String)),
        Field('unrelatedField', String),
    ...
    NonNullStringBox2Impl = ObjectType('NonNullStringBox2Impl', [
        Field('scalar', NonNullType(String)),
        Field('unrelatedField', String),
        Field('deepBox', SomeBox),
    ...
    Connection = ObjectType('Connection', [
        Field('edges', ListType(ObjectType('Edge', [
            Field('node', ObjectType('Node', [
                Field('id', ID),
                Field('name', String),
            ]))
        ])))
