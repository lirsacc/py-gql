...
from .types import (
    ObjectType, Field, Argument, NonNullType, ListType, EnumType, EnumValue,
    ScalarType, InputObjectType, UnionType, InterfaceType, is_abstract_type,
    WrappingType)
...
__Type__ = ObjectType(
    ...
    fields=lambda: [
        ...
        Field(
            ...
            args=[
                Argument('includeDeprecated', Boolean, default_value=False)
            ],
            resolve=lambda type_, args, *a, **kw: ([
                f for f in type_.fields
                if (not f.deprecated) or args.get('includeDeprecated')
            ] if isinstance(type_, (ObjectType, InterfaceType)) else None),
        ),
        Field(
            'interfaces',
        ...
        Field(
            ...
            ] if isinstance(type_, EnumType) else None),
        ),
        Field(
            'inputFields',
            ...
            resolve=lambda type_, *a, **kw: ([
                f for f in type_.fields
            ] if isinstance(type_, InputObjectType) else None),
        ),
        Field('ofType', __Type__, resolve=lambda type_, *a, **kw: (
...
__TypeKind__ = EnumType('__TypeKind', [
    EnumValue(
        'SCALAR',
    ...
    EnumValue(
        'UNION',
        description=('Indicates this type is a union. `possibleTypes` is '
                     'a valid field.')),
    EnumValue(
        'ENUM',
...
INTROPSPECTION_TYPES = (__Schema__, __Directive__, __DirectiveLocation__,
                        __Type__, __Field__, __InputValue__, __EnumValue__,
                        __TypeKind__,)
...
type_field = Field(
    ...
    args=[
        Argument('name', NonNullType(String)),
    ],
