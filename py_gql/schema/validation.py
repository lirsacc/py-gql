...
from .types import (
    Argument, Directive, EnumType, EnumValue, Field, InputField,
    InputObjectType, InterfaceType, is_input_type, is_output_type,
    NonNullType, ObjectType, UnionType)
...
def validate_schema(schema):
    ...
    for typ in schema.types.values():
        _assert(typ and hasattr(typ, 'name'),
                'Expected named type but got %s' % typ)
        _assert(is_introspection_type(typ) or _is_valid_name(typ.name),
                'Invalid type name "%s", '
...
def _is_valid_name(name):
    ...
    return bool(
        name and
        VALID_NAME_RE.match(name) and
        not name.startswith('__')
    )


def _assert_valid_name(name):
    _assert(_is_valid_name(name),
...
def validate_root_types(schema):
    ...
    if schema.query_type is not None:
        _assert(isinstance(schema.query_type, ObjectType),
    ...
    if schema.mutation_type is not None:
        _assert(isinstance(schema.mutation_type, ObjectType),
    ...
    if schema.subscription_type is not None:
        _assert(isinstance(schema.subscription_type, ObjectType),
...
def validate_directives(schema):
    ...
    for directive in schema.directives.values():
        _assert(isinstance(directive, Directive),
...
def validate_interfaces(schema, typ):
    ...
    for interface in typ.interfaces:
        _assert(isinstance(interface, InterfaceType),
                'Type "%s" mut only implement InterfaceType but got "%s"'
                % (typ, interface))

        _assert(interface.name not in imlemented_types,
...
def validate_implementation(schema, typ, interface):
    ...
    for field in interface.fields:
        ...
        _assert(object_field is not None,
                'Interface field "%s.%s" is not implemented by type "%s"'
                % (interface.name, field.name, typ))

        _assert(schema.is_subtype(object_field.type, field.type),
                'Interface field "%s.%s" expects type "%s" but '
                '"%s.%s" is type "%s"'
                % (interface.name, field.name, field.type,
                   typ, field.name, object_field.type))

        for arg in field.args:
            object_arg = object_field.arg_map.get(arg.name, None)

            _assert(object_arg is not None,
                    'Interface field argument "%s.%s.%s" is not provided by '
                    '"%s.%s"'
                    % (interface.name, field.name, arg.name, typ.name,
                       field.name))

            # TODO: Should this use is_subtype ?
            _assert(arg.type == object_arg.type,
                    'Interface field argument "%s.%s.%s" expects type "%s" but '
                    '"%s.%s.%s" is type "%s"'
                    % (interface.name, field.name, arg.name, arg.type,
                       typ.name, field.name, arg.name, object_arg.type))
...
def validate_input_fields(schema, input_object):
    ...
    for field in input_object.fields:
        ...
        _assert(isinstance(field, InputField),
                'Expected InputField in "%s" but got "%s"'
                % (input_object, field))

        _assert(field.name not in fieldnames,
                'Duplicate field "%s" on "%s"' % (field.name, input_object))

        _assert(is_input_type(field.type),
